...
const UserList = ({ match, history, friends, posts, users, loadAllUsersAction }) => {
    ...
    const cols = [
        ...
        },
        {
            ...
            accessor: 'verified',
            cellClass: 'text-muted  w-5',
            Cell: (props) => <><Badge color={props.value === 1 ? 'outline-success' : 'outline-danger'} pill className="mb-1">{props.value === 1 ? 'Verified' : 'Unverified'}</Badge></>,
        },
        {
            Header: 'Actions',
            accessor: 'action',
    ...
    const recomposeUsers = () => {
        ...
        const verifyUsers = users.filter(user => user.card_img_url || user.card_number);
        for (let user of verifyUsers) {
            let user_item = {};
            // copy all key-values
            for (let key in user) {
    ...
    const getIndexInImgViewer = (user_id) => {
        ...
        for (let [i, cardImg] of cardImgs.entries()) {
